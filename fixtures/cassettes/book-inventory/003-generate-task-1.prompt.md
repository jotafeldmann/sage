...
Create src/books.ts exporting a Book type with title, author, publicationYear and category, plus a small local dataset. The analysis confirms there is no data layer, so the dataset is defined in the module.
