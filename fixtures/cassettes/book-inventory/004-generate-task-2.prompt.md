...
- task-1: Added the Book type with title, author, publicationYear and category, plus three seed books. (src/books.ts)
...
src/books.ts exports:
  interface Book
  const books: Book[]

## Your task (2/4)

Create src/BookInventory.tsx as a named-export function component rendering each book's title, author, publication year and category, with a case-insensitive title search, a control to sort by publication year, and an empty state when the filter matches nothing.
